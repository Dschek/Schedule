...
import tydi.ru.schedule.mvc.model.*;
        ...
        try {
            ...
            schedule.stream()
                    ...
                    .forEach(elem -> {
                                ...
                                schedules.add(new ScheduleResponse(elem.getSubject().getName(), elem.getSubject().getTeacher().getFullName(), elem.getGroup().getName(), elem.getLessonNumber(), elem.getAuditorium()));
                            }
                    );
        }catch (Exception e){
